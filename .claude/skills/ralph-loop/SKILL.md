---
name: ralph-loop
description: "Use this skill when the user explicitly asks for an autonomous delegated loop that repeatedly runs a skill or prompt, gates each cycle, and stops at an `iterations:N` or `tasks:N` threshold."
version: 1.0.0
---
...
`/ralph-loop <skill-or-prompt> <threshold>`

Examples:

- `/ralph-loop start-task iterations:3`
- `/ralph-loop start-task tasks:2`
- `/ralph-loop "audit docs links" iterations:1`
...
- `$HOME/.claude/skills/<TASK>/SKILL.md`
...
- launch one fresh worker agent/subprocess using the available Claude environment
